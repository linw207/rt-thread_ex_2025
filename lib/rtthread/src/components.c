...
void rt_application_init(void)
{
    ...
    static char thread_tick_stack[1024];
    static struct rt_thread thread_tick;
    rt_thread_startup(tid);

    rt_thread_init(&thread_tick,
                   "thread_tick",
                   tick_init,
                   RT_NULL,
                   &thread_tick_stack[0],
                   sizeof(thread_tick_stack), 11, 5);
    rt_thread_startup(&thread_tick);
}
